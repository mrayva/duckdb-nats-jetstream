...
if [ "${RESET_STREAMS:-0}" = "1" ]; then
  ...
  for stream in telemetry environmental telemetry_proto events copy_out copy_roundtrip copy_roundtrip_const stats_gaps ingest_resume ingest_redelivery; do
...
# Create copy_out stream for COPY TO regression tests
"$NATS_BIN" stream add copy_out \
  --subjects "copy_out.>" \
  --storage file \
  --retention limits \
  --max-msgs=-1 \
...
# Create copy_roundtrip_const stream for constant-subject round-trip tests
"$NATS_BIN" stream add copy_roundtrip_const \
  --subjects "copy_roundtrip_const.>" \
  --storage file \
  --retention limits \
  --max-msgs=-1 \
  --max-bytes=-1 \
...
echo "Created stream: copy_roundtrip_const"
...
# Create a small deterministic stream for ingest redelivery tests
"$NATS_BIN" stream add ingest_redelivery \
  --subjects "ingest_redelivery.>" \
  --storage file \
  --retention limits \
  --max-msgs=-1 \
  --max-bytes=-1 \
...
echo "Created stream: ingest_redelivery"

"$NATS_BIN" pub --jetstream --quiet --count 4 --server="${NATS_URL}" ingest_redelivery.items "redelivery-test-{{Count}}" >/dev/null

echo "Seeded stream: ingest_redelivery with 4 messages"
